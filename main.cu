...
#define final_time 1e-10//2e-8
#define domain 1e4
#define protein_radius 1000
...
void runSimulation(Initial_Conditions ic)
{
  ...
  output_file << i_temp << ' ' << num_of_particles <<' ' << time_step <<' '<< protein_mass <<' '<< protein_radius <<' '<< water_mass <<' '<< water_radius << std::endl;
    ...
    Particle bp0, bp;
    cudaMemcpy(&bp0, d_particles, sizeof(Particle), cudaMemcpyDeviceToHost);
    output_file << 0 << ' ' << bp0.position.x <<' ' << bp0.position.y << ' ' << bp0.position.z << '\n';
    float t = 0;
    int i = 1;
    while(t < ic.tf)
    {
      ...
      t += ic.dt;
      if(i % frame_skip == 0){
        cudaMemcpy(&bp, d_particles, sizeof(Particle), cudaMemcpyDeviceToHost);
        output_file << t << ' ' << bp.position.x <<' ' << bp.position.y << ' ' << bp.position.z << '\n';
      }
      i++;
      // destroy the streams
